...
public enum AlipayField {
	...
	//== 公共参数
	APP_ID("app_id", "支付宝分配给开发者的应用ID"),
	
	METHOD("method", "接口名称"),
	
	FORMAT("format", "数据传输格式，仅支持JSON"),
	
	CHARSET("charset", "请求使用的编码格式"),
	
	SIGN_TYPE("sign_type", "签名类型"),
	
    SIGN("sign", "签名"),

    TIMESTAMP("timestamp", "发送请求的时间, 格式 yyyy-MM-dd HH:mm:ss"),
	
	VERSION("version", "调用的接口版本，固定为：1.0"),
	...
	TIMEOUT_EXPRESS("timeout_express", "设置未付款支付宝交易的超时时间"),
	
	TOTAL_AMOUNT("total_amount", "订单总金额"),
	
	SELLER_ID("seller_id", "卖家支付宝账户号"),
	
	PRODUCT_CODE("product_code", "销售产品码，商家和支付宝签约的产品码，为固定值QUICK_MSECURITY_PAY"),
	
	GOODS_TYPE("goods_type", "商品类型"),
	
	PASSBACK_PARAMS("passback_params", "公用回传参数"),
	
	PROMO_PARAMS("promo_params", "优惠参数"),
	
	EXTEND_PARAMS("extend_params", "业务扩展参数"),
	
	ENABLE_PAY_CHANNELS("enable_pay_channels", "可用渠道"),
	
	DISABLE_PAY_CHANNELS("disable_pay_channels", "禁用渠道"),
